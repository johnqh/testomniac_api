 ...
 */
export const starterSchema = pgSchema("testomniac");
 ...
 *
 * Indexes:
 * - `testomniac_histories_user_idx` on `user_id` for efficient user-scoped queries
 */
export const histories = starterSchema.table(
  ...
  (table) => ({
    userIdx: index("testomniac_histories_user_idx").on(table.user_id),
  })
);
