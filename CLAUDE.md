...
Backend API server for the Testomniac application.

**Package**: `testomniac_api` (private, not published)
...
- **testomniac_types** — Shared TypeScript type definitions; this project imports request/response types and helpers from it
- **testomniac_client** — API client SDK that consumes this server's HTTP endpoints
- **testomniac_lib** — Business logic library that uses testomniac_client to talk to this API
- **testomniac_app** — Web frontend that calls this API (default API URL mismatch: app uses 3001, API uses 8022)
- **testomniac_app_rn** — React Native app that calls this API
...
- All API responses must be wrapped in `BaseResponse<T>` using `successResponse()` or `errorResponse()` from `@sudobility/testomniac_types`
